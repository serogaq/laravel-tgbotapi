...
class BotManager {
	...
	public static function selectBot(string $username): ?Bot {
		...
		if(!isset(self::$botConf->log_channel) || !is_string(self::$botConf->log_channel) || self::$botConf->log_channel === 'default') self::$botConf->log_channel = config('logging.default', 'single');
		if(!isset(self::$botConf->api_server) || !is_string(self::$botConf->api_server)) self::$botConf->api_server = config('tgbotapi.api_server', 'https://api.telegram.org');
		return self::getBot();
	}
	...
	private static function getBot(): ?Bot {
		...
		if(is_null($botConf)) return null;
		if(isset(self::$bots[$botConf->username])) return self::$bots[$botConf->username];
		$bot = new Bot($botConf);
