...
class SetWebhook extends Command {
	...
	public function handle() {
		...
		foreach($res as $key => $value) $this->line("  $key: $value");
		$url = $this->option('url');
		if(is_null($url)) {
			while(true) {
				...
				if(!$this->confirm("The URL for the webhook will be used: $url", true)) continue;
				break;
			}
		}
		$data = ['url' => $url];
		if($res->url !== '') {
			if($this->confirm("Drop Pending Updates?", false)) $data['drop_pending_updates'] = true;
		}
