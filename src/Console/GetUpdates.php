...
class GetUpdates extends Command {
	...
	public function work(): void {
		if($this->runs > 500 && Carbon::now()->second >= 57) $this->completed();
		try {
	...
	protected function getBatchId(): string {
		if(!isset($this->batchId)) $this->batchId = substr(md5(random_bytes(5)), 0, 7);
		return $this->batchId;
	}
	...
	protected function shouldContinue(): bool {
		if($this->shouldRunOnce() && $this->runs === 1) return false;
		if($this->shouldRunUntilComplete() && $this->complete) return false;
		return true;
	}
	...
	private function shouldRunOnce(): bool {
		if($this->option('once') === true) return true;
		return false;
	}
	...
	private function shouldRunUntilComplete(): bool {
		if($this->option('until-complete') === true) return true;
		return false;
	}
	...
	/*public function handle() {
		...
		while(time() < $end_time) {
			...
			try {
				...
				Cache::store('file')->put('tgbotapi:getupdates:forked', '0');
				$this->fork(true);
				return 0;
			}
		}
