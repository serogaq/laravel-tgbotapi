...
class InstallTgBotApiTest extends TestCase {
	...
	/** @test */
	function package_installation_successful() {
		if(File::exists(config_path('tgbotapi.php'))) unlink(config_path('tgbotapi.php'));
		if(File::exists(app_path("Listeners/UpdateProcessing.php"))) unlink(app_path("Listeners/UpdateProcessing.php"));
		if(!empty(realpath(storage_path('tgbotapi')))) $this->_removeDirectory(storage_path('tgbotapi'));
	...
	/** @test */
	function already_installed_package_the_user_may_not_want_to_reinstall() {
	...
	/** @test */
	function already_installed_package_the_user_may_want_to_reinstall() {
		File::put(config_path('tgbotapi.php'), '<?php /*TEST*/ return []; ?>');
		if(File::exists(app_path("Listeners/UpdateProcessing.php"))) unlink(app_path("Listeners/UpdateProcessing.php"));
