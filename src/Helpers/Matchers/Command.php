...
class Command extends SimpleXMLElement {
    ...
    private static function prepareXml(?string $command, CommandUpdate $update, int $args): string {
        if(preg_match('#^/(?<command>[a-zA-Z0-9_]+)(@[a-zA-Z0-9_]+bot)?(?<data> .*)?$#msu', $update->asArray()['message']['text'], $matches, PREG_OFFSET_CAPTURE) === 1) {
            ...
            if ($args === self::WITHOUT_ARGS) {
                ...
                if (!empty($data)) return '<!--' . htmlentities(json_encode($data)) . '--><command>1</command>';
                else return '<command/>';
            } elseif ($args === self::WITH_SPACE_ARGS) {
                if ($command === $matchCommand) {
                    if (isset($matches['data']) && !empty(trim($matches['data'][0]))) return '<!--' . htmlentities(json_encode([' ' => trim($matches['data'][0])])) . '--><command>1</command>';
                    else return '<command>1</command>';
                } else return '<command/>';
    ...
    public function getMatches(): array|string|null {
        ...
        if(count($result) === 1) return isset($result[' ']) ? $result[' '] : $result['_'];
        return $result;
    }
